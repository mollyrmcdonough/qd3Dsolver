...
'm_hh':0.34, # Vurgaftman2001
'm_lh':0.0447, # Vurgaftman2001
'epsilonStatic':15.69,
'Eg':0.726,#Eg(0K)=0.812eV, Varshni-corrected to 300K (alpha=0.417e-3, beta=140), Vurgaftman2001
'Ep':27.0,
'F':-1.63,
'Band_offset':0.63,# placeholder: not used unless GaSb is used as a standalone layer (it
                    # currently only appears here as an alloy component, e.g. InGaSb)
'm_e_alpha':0.0,
'GA1':13.4,#luttinger parameter, Vurgaftman2001
'GA2':4.7,
'GA3':6.0,
'C11':88.42,
'C12':40.26,
'a0':6.0959,
'a0_sub':6.0959,# defaults to GaSb's own a0 (no assumed strain); override per-simulation to
                 # whatever the actual growth/virtual substrate is
'Ac':-9.33,
'Av':-1.32,
'B':-2.0,
'delta':0.76,
...
'VSATN':3e5,# Saturation Velocity of Electrons
'VSATP':6e5, # Saturation Velocity of Holes
'AVb_E':-6.25#Average Valence Band Energy or the absolute energy level
},
'InSb':{
'm_e':0.0135, # Vurgaftman2001
'm_hh':0.405, # Vurgaftman2001
'm_lh':0.0162, # Vurgaftman2001
'epsilonStatic':17.5,
'Eg':0.174,#Eg(0K)=0.235eV, Varshni-corrected to 300K (alpha=0.32e-3, beta=170), Vurgaftman2001
'Ep':23.3,
'F':-0.23,
'Band_offset':2.22,# Derived (not a 0-1 fraction): calibrated so fi_e/fi_h reproduce the
                    # absolute InAs/GaSb-family (type-II/broken-gap) alignment, the same way
                    # as the InGaSb entry below -- see that entry's comment for the method.
                    # Anchored to this file's InAs entry via the Zunger valence-band scale in
                    # database1.py (InAs 1.390, InSb 1.750 eV).
'm_e_alpha':0.0,
'GA1':34.8,#luttinger parameter, Vurgaftman2001
'GA2':15.5,
'GA3':16.5,
'C11':68.47,
'C12':37.35,
'a0':6.4794,
'a0_sub':6.4794,# defaults to InSb's own a0 (no assumed strain); override per-simulation to
                 # whatever the actual growth/virtual substrate is (see the example notebooks)
'Ac':-6.04,
'Av':0.31,
'B':-2.0,
'delta':0.81,
...
'VSATN':3e5,# Saturation Velocity of Electrons
'VSATP':6e5, # Saturation Velocity of Holes
'AVb_E':-6.09#Average Valence Band Energy or the absolute energy level
}
}
...
'InGaSb':{#In_x Ga_1-x Sb, x = alloy fraction = In content
'Bowing_param':0.415,#Gamma-valley bandgap bowing, Vurgaftman2001
'Band_offset':1.86,# NOT a physical 0-1 ratio: chosen so that fi_e/fi_h reproduce the
                    # absolute InAs/GaSb-family band alignment (see note below), calibrated
                    # around x~0.6-0.7. InAs/Sb-based III-Vs form a type-II ("broken-gap"
                    # family) heterojunction, unlike the type-I AlGaAs/GaAs-style alloys
                    # this database was originally built around, so the usual 0-1
                    # "fraction of the gap" reading of Band_offset does not apply here.
                    # Derived from the valence-band alignment table in database1.py
                    # (A. Zunger scale: InAs 1.390, GaSb 1.777, InSb 1.750 eV), anchored to
                    # this file's existing InAs entry (Band_offset=0.63, Eg=0.4). Re-derive
                    # if you need a different literature value for the InAs/GaSb natural
                    # valence band offset (commonly cited in the range 0.4-0.51 eV).
'm_e_alpha':0.0,
'delta_bowing_param':0.0,
'a0_sub':6.0583,# InAs lattice constant: this alloy is used here as a well grown on InAs
'Material1':'InSb',
'Material2':'GaSb',
'TAUN0':0.1E-6,
'TAUP0':0.1E-6,
...
'InAsSb':{#InAs_x Sb_1-x, x = alloy fraction = As content (NOT Sb content -- the
          # composition is usually quoted by Sb content in the literature, so convert:
          # x_As = 1 - x_Sb)
'Bowing_param':0.67,#Gamma-valley bandgap bowing, Vurgaftman2001
'Band_offset':1.656,# NOT a physical 0-1 ratio -- see the InGaSb entry above for why and how
                     # this is derived. Calibrated around x_As~0.4 (x_Sb~0.6, near InAsSb's
                     # minimum-bandgap composition) from the same Zunger valence-band scale
                     # in database1.py, anchored to this file's InAs entry.
'm_e_alpha':0.0,
'delta_bowing_param':0.0,
'a0_sub':6.3109,# defaults to InAsSb's own (x_As=0.4) a0 (no assumed strain); override
                # per-simulation to whatever the actual growth/virtual substrate is
'Material1':'InAs',
'Material2':'InSb',
'TAUN0':0.1E-6,
'TAUP0':0.1E-6,
